...
export function ExportPage() {
  ...
  const onExportBatch = async () => {
    ...
    if (!result || result.succeeded === 0) return;
    ...
    const outputFolder =
      count === 1 && lastSuccess
        ? folderFromOutputPath(lastSuccess.output_path)
        : settings.output_folder;

    if (outputFolder) {
      notifyExportSuccess(outputFolder, result.succeeded);
    }
  };
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <aside className="flex w-80 shrink-0 flex-col overflow-y-auto p-4">
            ...
            <div className="flex flex-col gap-1">
              ...
              {selectedPhotoIds.length === 1 ? (
                <p className="text-xs text-ae-text-secondary">
                  Single photo exports save to an{" "}
                  <span className="text-ae-text-primary">Exports</span> folder beside the source
                  image.
                </p>
              ) : (
          ...
          {isExporting && (
            <div className="mt-4 flex items-center gap-2 text-xs text-ae-text-secondary">
              <Spinner size={16} />
              <span>
                Exporting {selectedPhotoIds.length} photo
          ...
          <div className="mt-6 flex flex-col gap-2">
            <button
              ...
              disabled={
                ...
                (selectedPhotoIds.length > 1 && !settings.output_folder)
              }
              onClick={() => void onExportBatch()}
              className="flex items-center justify-center gap-2 rounded bg-ae-accent px-4 py-2 text-sm text-white disabled:opacity-40"
            >
              {isExporting && <Spinner size={14} className="border-white/30 border-t-white" />}
