...
pub fn decode(path: &Path) -> Result<DecodedImage, DecodeError> {
    ...
    // Neutral decode — develop stack applies WB from sliders (avoids double white-balance).
    const NEUTRAL_WB: [f32; 4] = [1.0, 1.0, 1.0, 1.0];
    let wb = NEUTRAL_WB;
    ...
    Ok(DecodedImage::new(w as u32, h as u32, rgb))
}
